...
def get_storage_bucket(bucket_name: Optional[str] = None):
    ...
    candidates = [
        ...
        "orthofinixai.firebasestorage.app",
        "orthofinixai.appspot.com"
    ]
    for c in candidates:
        if c:
            try:
...
def upload_clinical_image(
    ...
    """
    Uploads clinical image bytes to Firebase Cloud Storage at 'cases/{uid}/{case_id}/images/{view_type}/{filename}'
    ...
    if case_id and view_type:
        blob_path = f"cases/{clean_uid}/{case_id}/images/{view_type}/{clean_filename}"
    elif case_id:
        blob_path = f"cases/{clean_uid}/{case_id}/{clean_filename}"
    else:
        blob_path = f"cases/{clean_uid}/uploads/{clean_filename}"

    # Try uploading to Firebase Cloud Storage buckets
    bucket_names = [
        os.getenv("FIREBASE_STORAGE_BUCKET"),
        "orthofinixai.firebasestorage.app",
        "orthofinixai.appspot.com"
    ...
    for b_name in bucket_names:
        ...
        try:
            ...
            print(f"[Firebase Storage] Uploaded to {b_name}/{blob_path} -> {public_url}")
            return public_url
        except Exception as err:
            print(f"[Firebase Storage] Notice for bucket '{b_name}': {err}")

    # Fallback to local server serving if storage bucket is pending activation
    ...
    with open(local_path, "wb") as f:
        f.write(file_bytes)
    fallback_url = f"{base_url}/uploads/{clean_filename}"
    print(f"[Storage Fallback] Saved locally: {local_path} -> {fallback_url}")
    return fallback_url
